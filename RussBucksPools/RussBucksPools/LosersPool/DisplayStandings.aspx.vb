...
Public Class DisplayStandings
                ...
                Using (_dbLoserPool)
                    Using (_DbPools)
                        Using (_dbApp)
                            ...
                            Dim timePeriodIncrement = CInt(queryPoolParam.timePeriodIncrement)
                            Dim thisTimePeriod = queryPoolParam.TimePeriod
                            Dim timePeriodName = queryPoolParam.timePeriodName

                            If thisTimePeriod <> staleTimePeriod Then
                            ...
                            Dim querySeasonStart = (From time1 In _dbLoserPool.ScheduleTimePeriods
                                                    Where time1.TimePeriod = timePeriodName + "1" And time1.CronJob = cronJobName).Single

                            Dim seasonStartTime = DateTime.Parse(CDate(querySeasonStart.startGameDate) + " " + CDate(querySeasonStart.startGameTime))

                            If seasonStartTime > currentDateTime Then
                                Response.Redirect("~/LosersPool/NoPreseasonStandings.aspx")
                            End If

                            Dim queryLosers = (From loser1 In _dbLoserPool.LoserList
                                               Where loser1.PoolAlias = poolAlias And loser1.CronJob = cronJobName
                                               Order By loser1.TimePeriod Descending).ToList

                            Dim queryTimePeriods = (From timePeriod1 In _dbLoserPool.ScheduleTimePeriods
                                               Where Not timePeriod1.TimePeriodStartDate Is Nothing And timePeriod1.CronJob = cronJobName
    ...
    Private Sub CreateLoserTable(thisTimePeriod As String, poolAlias As String, queryTimePs As List(Of ScheduleTimePeriod), LoserCollectionSorted As Dictionary(Of String, Loser), cronJobName As String)
        SortLosers(thisTimePeriod, poolAlias, queryTimePs, cronJobName)
